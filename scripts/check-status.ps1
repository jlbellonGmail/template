...
function Git([string[]]$A) { $o = & git @A 2>&1; if ($LASTEXITCODE) { throw "git command failed" }; ($o | ForEach-Object { $_.ToString() }) -join "`n" | ForEach-Object { $_.Trim() } }
...
try {
    $root=Git @("rev-parse","--show-toplevel");$path=Join-Path $root "STATUS.md"
    ...
    $branch=Git @("branch","--show-current");if(!$branch){$branch="(detached)"};$head=Git @("rev-parse","HEAD");$errors=@();$warnings=@()
    ...
    $tree=if((Git @("status","--porcelain"))){"dirty"}else{"clean"};if((Field $block "Working tree")-ne$tree){$warnings+="STALE working tree: snapshot regenerable"}

...
- **T-01** — Redactar en `AGENTS.md`, sección "Setup manual (una sola
  ...
  aprobación, dismiss stale approvals) aplicados a `develop`, el comando
  `gh api` completo con el payload JSON, el paso de validación local
  `ConvertFrom-Json`, el comando de verificación de solo lectura, la
  nota de dependencia con la feature `04-ci-wiring-product-tests`, y la
  alternativa manual de UI.
  ...
  explícitamente `dismiss_stale_reviews`, `required_approving_review_count`,
  `contexts` (o `checks`) y `develop`.
  Traza: AC-1, AC-2, AC-3.
...
- **T-03** — Escribir `docs/tecnica/operational-readiness-docs.md` con
  ...
  `develop` y no `main`, por qué `enforce_admins: false`, por qué el
...
- **T-04** — Escribir `docs/usuario/operational-readiness-docs.md` con
  ...
  el comando de branch protection, qué hacer si el reconciliador local
  queda bloqueado).
  Verificación: el archivo existe y no está vacío.
  Traza: AC-6.

- **T-05** — Crear `runs/05-operational-readiness-docs/decision.md` con
  decisiones demostrables desde spec/plan/tasks/auditoría/
  implementación de T-01 a T-04, sin afirmar aprobación de merge.
  Verificación: el archivo existe, no está vacío, y referencia
  explícitamente las decisiones de T-01/T-02 (branch protection y
  troubleshooting EDR).
  Traza: AC-7.
